...
<div class="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
    ...
    <div>
        <h1 class="text-xl md:text-2xl font-semibold text-slate-100">Edit Account</h1>
        <p class="text-sm text-slate-400">{{ $account->code }} — {{ $account->name }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('modules.accounting.chart.update', $account) }}"
        ...
        <div>
            ...
            <input name="code" value="{{ old('code', $account->code) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label class="flex items-center gap-2 text-sm text-slate-200">
                <input type="checkbox" name="is_control_account" value="1"
                       @checked(old('is_control_account', $account->is_control_account ?? 0) == 1)
            ...
            <label class="flex items-center gap-2 text-sm text-slate-200">
                <input type="checkbox" name="is_active" value="1"
                       @checked(old('is_active', $account->is_active ?? 1) == 1)
                       class="rounded bg-black/30 ring-1 ring-white/10">
                Active
            </label>
        </div>
